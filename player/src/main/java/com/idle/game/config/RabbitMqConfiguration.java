...
public class RabbitMqConfiguration implements RabbitListenerConfigurer {
    ...
    @Value("${idle.config.useResource.sendDLQQueue}")
    private String useResourceDLQQueue;
    @Value("${idle.config.useResource.sendQueue}")
    private String useResourceQueue;
     ...
     */
    @Bean
    public DirectExchange useResourceExchange() {
        return new DirectExchange(useResourceExchange, true, false);
    }
     ...
     */
    @Bean
    public Queue useResourceQueue() {
        return QueueBuilder.durable(useResourceQueue)
                .withArgument("x-dead-letter-exchange", "")
                .withArgument("x-dead-letter-routing-key", useResourceDLQQueue)
                .build();
    }
     ...
     */
    @Bean
    public Queue useResourceDLQQueue() {
        return new Queue(useResourceDLQQueue, true, false, false);
    }
     ...
     */
    @Bean
    public Binding useResourceBinding() {
        return BindingBuilder.bind(useResourceQueue()).to(useResourceExchange()).with(useResourceQueue);
    }
